...
import ButtonSvg from "../../../classes/input/buttonSvg";
import svg from "../../../assets/categories/image-pen.svg";
import GroupLayout from "../../../classes/layout/groupLayout";
...
const CanvasArea = ({
        ...
        sortedElements.forEach(async (element) => {
          ...
          try {
            ...
            if (fabricElement) {
              ...
              console.log(fabricElement.getBoundingRect());
              setTracking(false);
  ...
  const AddToLayout = (movingElement) => {
    // Add mouse-up event listener
    canvasObj.on("mouse:up", () => {
      // Get all objects from the canvas except the moving element
      const objects = canvasObj.getObjects().filter((obj) => {
        return obj.id !== movingElement.id; // Add 'return' here
      });

      // Get the left and top properties of the moving element
      const movingLeft = movingElement.left;
      const movingTop = movingElement.top;

      let isDroppedOnElement = false; // Flag to track if the moving element is dropped on any element
      let selectedElement = null;

      // Iterate through all objects to check for overlap
      for (let i = 0; i < objects.length; i++) {
        const obj = objects[i];
        if (obj.customType === "GroupLayout") {
          let objectRect = obj.getBoundingRect();
          ...
          // Check if movingElement is dropped on this object
          if (
            movingLeft >= objLeft &&
            movingLeft <= objRight &&
            movingTop >= objTop &&
      ...
      if (isDroppedOnElement && selectedElement.customType === "GroupLayout") {
        const updatedElements = elements.map((el) => {
          // Check if the current element's id matches the selected element
          if (el.id === selectedElement.id) {
            // Ensure `el.elements` is an array and add `elementData` to it
            const updatedElementsList = Array.isArray(el.arr)
              ? [...el.arr, elementData]
              : [elementData];

            // Return the updated element object with the new `elements`
            return { ...el, arr: updatedElementsList };
          }
          return el; // If IDs don't match, return the unmodified element
        });

        // Exclude the element with the same id as `elementData`
        const finalElements = updatedElements.filter(
          (el) => el.id !== movingElement.id
        );

        console.log(finalElements);
        const newPositions = Object.fromEntries(
          Object.entries(positions).filter(([key]) => key !== elementData.id)
        );

        setPOSITION(newPositions);
        // Now set the updated elements array to the state
