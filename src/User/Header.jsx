...
function Header({ onGenerateCode, onDownloadProject, onWindowSizeChange }) {
  ...
  const handleWindowSizeChange = () => {
    if (width > 800) {
      setWidth(800);
      let temp = 800;
      onWindowSizeChange(temp, height, color); // Calls the passed function with current width and height
    } else {
      onWindowSizeChange(width, height, color); // Calls the passed function with current width and height
    }
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between h-[70px] bg-gradient-to-l from-[#01A4C9] via-[#08B9E2ED] to-[#0FD1FF]">
        ...
        <div className="flex items-center justify-end px-6 w-[60%] h-full">
          <button
